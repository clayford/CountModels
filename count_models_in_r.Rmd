...
- linear model review
- distributions for counts: Poisson and Negative Binomial
...
1. the formula is a weighted sum of predictors (eg, 10 + 5x + 3z)
...
Linear modeling tries to recover the weights (or coefficients) in the first assumption (10 and 5) and the standard deviation in the 3rd assumption (10).
...
We could also plot our fitted line through the data since we only have two dimensions. The symbold `|>` is a "pipe" that takes the result of the function on the left and "pipes" it into the first argument of the function on the right.
...
Today we look at _count models_ where we model integers greater than or equal to 0 and assume a different distribution for the random error (ie, not a Normal distribution).

## Distributions for counts

Let's review two common distributions for count data. Count data are integers greater than or equal to 0. These are discrete values we would use to record counts of events. 

### Poisson distribution

The Poisson distribution is a discrete probability distribution with one parameter: _lambda_.

lambda is the mean; the _variance and mean are equal_ in Poisson distributions.
...
Above we plugged in a single mean. But perhaps the mean depends on some other variable(s). Perhaps the mean is lower for an "untreated" group than the mean for a "treated" group. This is a simple example of statistical modeling, which estimates the mean of a dependent variable conditional on values of predictor variables. 

Let's simulate a bunch of 0s and 1s to indicate untreated (0) and treated (1)
...
Now we randomly sample from a Poisson distribution with a mean of `exp(1.2 + 1.8*trt)`. The `exp()` function means exponentiate, or raise e to the power of the argument. Why use `exp()`? That ensures we get a positive mean. Lambda has to be positive. Here it's not necessary but as we'll see this transformation is built into count models.
...
The `link = "log"` argument is assumed by default but is included here for completeness. Why "log"? Because it's the _link_ to the additive model. Recall that taking the log of an exponentiated value returns the original value.
...
Let's use our model to simulate some data and see if it looks similar to our observed data set. Below we use `rpois()` again but this time with our model estimated coefficients. We then plot the original distribution of the raw data and compare it to the distribution of the model-simulated data.
...
This is confusing because "Dispersion parameter" in this sentence refers to the GLM dispersion parameter (phi), which is different from the negative binomial dispersion parameter (theta)! The GLM dispersion parameter gets estimated when we fit models with `family = quasipoisson(link = "log")`. (See Appendix: Modeling with quasipoisson)
...
- prestige: prestige of the PhD department
...
Let's fit a Poisson model using female and mentor as predictors. 
...
Each additional child under the age of 5 can expect to reduce the count of published articles by 7 to 19%.
...
The length of the bars are the _observed counts_. Notice the counts have been transformed with a square root transformation. That's to help smaller numbers of counts not get squashed by larger numbers of counts.

Bars that dip below 0 are _underfit_; bars that hang above 0 are _overfit_.
...
```
phd.pois <- glm(articles ~ female + mentor + kid5, data = d, family = poisson)
```
...
Let's fit a new Poisson model with more predictors.
...
The `glm.nb()` function in the MASS package fits a negative binomial model. The syntax `articles ~ .` models articles as a function of all other columns in the PhdPubs data frame.
...
It certainly seems like the negative binomial model is better than a Poisson model.
...
A common approach to modeling such data is to create a mixture of two models to account for the _extra zeroes_:
...
Now let's fit a model that accommodates excess zeroes using the `zeroinfl()` function from the pscl package. This fits two models, separated by a `|`
...
Notice the intercept is nowhere close to the true value of 3. It is the estimated _rate_ when when trt = 0.
...
## Appendix: Modeling with quasipoisson
