...
- rnorm(n = 25, mean = 11.135 + 5.042*x, sd = 9.7)

This is close to the "true" values we used to generate the data:

- rnorm(n = 25, mean = 10 + 5*x, sd = 10)
...
The Poisson distribution is a discrete probability distribution with one parameter: lambda.
...
Sample data from a Poisson distribution with a lambda of 3. Note that lambda can take decimal values.
...
The Negative binomial distribution is also a discrete probability distribution, but with two parameters: mu (mean) and theta (a dispersion parameter)
...
Once we exponentiate, we can multiply by 100 and subtract 1 to get the percent increase or decrease.
