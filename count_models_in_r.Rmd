...
- using effect plots to visualize models with interactions
...
Count data are integers greater than or equal to 0. Let's review two common probability distributions for count data. 
...
Visualize distribution of counts. The `|>` symbol is the _pipe operator_. It takes the output of a function and passes it to the first argument of the next function. To insert pipe operator: Ctrl + Shift + M (Win/Linux) or Shift + Command + M (Mac)

```{r}
table(y1) |> plot()
# same as plot(table(y1))
```


We can also visualize the proportion of counts. The proportions sum to 1.
...
```{r}
exp(1.2 + 1.8*0) # about 3
```

expected count when trt = 1

```{r}
exp(1.2 + 1.8*1) # about 20
```
...
We use the `glm()` function to fit a poisson count model. Use the same way as `lm()`, but include the family argument: `family = poisson(link = "log")` 
...
Exponentiating the _slope_ coefficient tells us the _multiplicative effect_ of trt == 1. Expected count of y when trt == 1 is about 6.1 times the expected count when trt == 0.
...
Fit a model that attempts to "recover" the true values in the model specified above. Call the model 'mod2'
...
See this article for more information on deviance residuals:
...
```{r}
null_m1 <- glm(y_resp ~ 1, family = poisson)
...
The Residual deviance is lower than the Null deviance, suggesting the female and mentor variables help explain some of the variability in article count and are at least better than simply taking the mean (ie, the null intercept-only model).
...
The expected number of publications for female candidates is about 0.83 times that of male candidates, or about 17% fewer.
...
Confidence intervals give us a sense of how certain these estimated effects are. Since we exponentiate and get a multiplicative effect, _overlapping 1_ (instead of 0) indicates an uncertain direction in magnitude.
...
_We should check the fit of the model before putting too much stock into these interpretations_. That's what we do next.
...
We can use these coefficients to generate data from a poisson distribution and see how it compares to the original data.

```{r}
sim_data <- rpois(n = 915, lambda = exp(0.43673257 + 
                    -0.24310649*(d$gender == "female") + 
                    0.02564518*d$mentor + 
                    -0.14378697*d$kids))
# compare plots
op <- par(mfrow = c(1,2))
table(sim_data) |> plot(xlab = "")
table(d$articles) |> plot()
par(op)
```

An easier way is to use the `simulate()` function:

```{r}
op <- par(mfrow = c(1,2))
table(simulate(phd.pois2)[[1]]) |> plot(xlab = "")
table(d$articles) |> plot()
par(op)

```

An even better way to visualize model fit for count models is through _rootograms_.

The topmodels package provides the `rootogram()` function to quickly generate "hanging" rootograms to visualize goodness of fit.

```{r}
topmodels::rootogram(phd.pois2)
```
...
We can use the `xlim` argument to increase or decrease the count displayed

```{r}
topmodels::rootogram(phd.pois2, xlim = c(0, 12))
```
...
```{r}
topmodels::rootogram(phd.pois2, style = "standing")
```

The suspended version uses less ink and shows just the under and over-fit counts.

```{r}
topmodels::rootogram(phd.pois2, style = "suspended")
```

This is an ok not great fitting model. We are underfitting 0 counts and overfitting 1, 2, and 3 counts.
...
Diagnostic plots help us assess model fit. Recall an earlier negative binomial model with an interaction.

```{r}
formula(phd.nb2)
```

Let's demonstrate some basic model diagnostics using this model.

_Residual plots_ plot residuals versus fitted values or predictors. We hope residuals do not vary systematically with fitted values or predictors. The `residualPlot()` function from the {car} package makes creating these plots easy.

Residuals versus fitted values plot using _standardized_ residuals. Standardized residuals account for dispersion. This plot looks OK, but hints at problems for larger fitted values. The smooth trend line dipping below zero at the far right indicates our model probably shouldn't be trusted for estimating large article counts.

```{r}
library(car)
residualPlot(phd.nb2, type = "rstandard")
```

We can also look at residuals plots for predictors. Again these shouldn't exhibit any trends. Both of these plots look pretty good. 

```{r}
residualPlot(phd.nb2, "mentor", type = "rstandard")
```

```{r}
residualPlot(phd.nb2, "prestige", type = "rstandard")
```

The {car} package also provides the `influencePlot()` function to investigate _influential values_. Below row 328 has high leverage (hat value) and high influence (Cook's D). 

```{r}
influencePlot(phd.nb2)
```

Subject 328 only published one article but had a mentor who published 77 articles in the past 3 years while being in department with very low prestige. 

```{r}
d[328, c("articles", "mentor", "prestige")]
```

An influential observation may change the model when it's dropped from the data. Let's see how observation 328 influences the model. We update the original model by dropping row 328 and then compare model coefficients using the `compareCoefs()` function from the {car} package.

```{r}
phd.nb3 <- update(phd.nb2, subset = -328)
compareCoefs(phd.nb2, phd.nb3)
```

All coefficients change somewhat and the interaction gets a little bit bigger, but it doesn't seem like dropping this subject meaningfully changes the conclusions of the model. 



## Appendix: Non-linear effects
...
We can simulate data as before using the `rztpois()` function from the topmodels package, which simulates data from a zero-truncated poisson distribution.
...
Notice also the summary output for the quasipoisson model does not return an AIC estimate. That's because we have a quasi-likelihood, not a true likelihood. This is one reason why negative binomial models are usually preferred to model overdispersion in count models. 
