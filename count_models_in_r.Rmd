...
Count data are integers greater than or equal to 0. These are discrete values we might use to record counts of events. Let's review two common probability distributions for count data. 
...
Visualize distribution of counts. The `|>` symbol is the _pipe operator_. It takes the output of a function and passes it to the first argument of the next function.
...
One way to visualize model fit for count models is through _rootograms_.
...
```{r}
topmodels::rootogram(phd.pois, xlim = c(0, 15))
```
